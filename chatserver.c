 ...
 * Signal handler.
 */
void catch_signal(int sig) { running = false; }
...
void broadcast_buffer(int skip_index, char *buf) {
    uint16_t bytes_to_send = strlen(buf) + 1;
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (i != skip_index && client_sockets[i] != -1) {
            if (send_with_length(client_sockets[i], buf, bytes_to_send) == -1) {
                printf("%d\n", client_sockets[i]);
...
void cleanup() {
    ...
    snprintf(outbuf, sizeof(outbuf), "bye");
...
void disconnect_client(int index, char *ip, int port) {
    ...
    snprintf(outbuf, sizeof(outbuf), "User [%s] left the chat room.", usernames[index]);
...
int handle_server_socket() {
    ...
    int new_socket =
        accept(server_socket, (struct sockaddr *)&server_addr, &addrlen);
    ...
    snprintf(connection_str, sizeof(connection_str), "[%s:%d]", inet_ntoa(server_addr.sin_addr),
    ...
    create_welcome_msg();
    if (send_with_length(new_socket, outbuf, strlen(outbuf) + 1) == -1 &&
        errno != EINTR) {
        print_date_time_header(stderr);
        fprintf(stderr, "Warning: Failed to send welcome message. %s.\n",
                strerror(errno));
    } else {
        print_date_time_header(stdout);
    ...
    int bytes_recvd = recv_with_length(new_socket, inbuf, sizeof(inbuf));
    if (bytes_recvd == 0) {
    ...
    // Check username length and truncate at MAX_NAME_LEN if bigger
    if (strlen(inbuf) > MAX_NAME_LEN) {
        inbuf[MAX_NAME_LEN] = '\0';
    }
    ...
    for (int i = 0; i < MAX_CONNECTIONS; i++) {
        if (client_sockets[i] == -1) {
            ...
            num_connections++;
            snprintf(outbuf, sizeof(outbuf), "User [%s] joined the chat room.", usernames[i]);
...
void handle_client_socket(int index) {
    ...
    int bytes_recvd =
        recv_with_length(client_sockets[index], inbuf, sizeof(inbuf));
    if (bytes_recvd == 0) {
        // The client disconnected.
        disconnect_client(index, ip, port);
        return;
    } else if (bytes_recvd == -1 && errno != EINTR) {
        print_date_time_header(stderr);
        fprintf(stderr,
        ...
        if (strcmp(inbuf, "bye") == 0) {
            ...
            snprintf(outbuf, sizeof(outbuf), "[%s]: %s", usernames[index], inbuf);
            broadcast_buffer(index, outbuf);
        }
    }
}
